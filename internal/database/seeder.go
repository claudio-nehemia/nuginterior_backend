...
func SeedSettings(db *gorm.DB, logger *zap.Logger) {
	...
	SyncCompanySettings(db, logger)
}

// SyncCompanySettings ensures that all verified companies have the contract_clauses setting.
func SyncCompanySettings(db *gorm.DB, logger *zap.Logger) {
	var companies []entity.Company
	if err := db.Where("status = ?", "verified").Find(&companies).Error; err != nil {
		logger.Error("Failed to load companies for settings sync", zap.Error(err))
		return
	}
	...
	for _, c := range companies {
		var existing entity.Setting
		err := db.Where("company_id = ? AND key = ?", c.ID, "contract_clauses").First(&existing).Error
		if err == gorm.ErrRecordNotFound {
			db.Create(&entity.Setting{
				CompanyID:   c.ID,
				Key:         "contract_clauses",
				Value:       defaultClauses,
				Description: "Pasal-pasal yang ditampilkan dalam dokumen kontrak (format JSON)",
			})
			count++
		}
	}
	logger.Info("Synchronized company settings", zap.Int("companies_processed", len(companies)), zap.Int("new_settings_created", count))
}
